...
export default function TrustedBrands() {
  return (
    <section aria-label="Trusted travel brands" className="py-8 md:py-10">
      <Container className="flex flex-wrap items-center justify-center gap-x-6 gap-y-6 sm:gap-x-10 md:justify-between md:gap-x-4">
        {brands.map((brand) => (
          <div
            key={brand.name}
            className="flex h-8 items-center text-[#9A9A9A] transition-colors duration-300 hover:text-[#EDEDED] sm:h-10"
            title={brand.name}
          >
            {brand.mark}
          </div>
