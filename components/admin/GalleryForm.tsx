...
import { galleryCropSpec } from "@/lib/image-crop";
...
export default function GalleryForm({ mode, initial, action }: GalleryFormProps) {
  ...
  const [aspect, setAspect] = useState<GalleryAspect>(initial?.aspect ?? "landscape");
        ...
        <div className="mt-5 space-y-4">
          <label className="block">
            ...
            <select
              name="aspect"
              value={aspect}
              onChange={(event) => setAspect(event.target.value as GalleryAspect)}
              className={fieldClass}
            >
              {galleryAspects.map((item) => (
            ...
            <p className="mt-1.5 text-xs text-[#7A7A7A]">
              Pick the shape first, then upload so the crop matches the gallery tile.
            </p>
          </label>
