...
import { importSampleGalleryAction, type ActionState } from "@/app/admin/actions";
import { cn } from "@/lib/utils";

export default function ImportSampleGalleryButton({ compact = false }: { compact?: boolean }) {
  const [state, action, pending] = useActionState(importSampleGalleryAction, null as ActionState);
  ...
  return (
    <form action={action} className={cn("flex flex-col", compact ? "items-stretch" : "items-start gap-2")}>
      ...
      >
        {pending ? "Importing..." : "Import sample photos"}
      </button>
